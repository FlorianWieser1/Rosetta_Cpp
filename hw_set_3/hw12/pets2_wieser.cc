...
// If a function is declared as "virtual" in a base class, derived classes are allowed
// to re-define (change functionality but not the interface of the function itself) it.
//
...
public:
  ...
  string get_favorite_food() const;
  virtual void set_favorite_food( string favorite_food ); // Add the two methods, set_favorite_food and get_favorite_food here
...
private:
  ...
  string favorite_food_;  // Add a data member here to store the Pet's favorite food
...
string
Pet::get_favorite_food() const { return favorite_food_; }
...
void
Pet::set_favorite_food( string favorite_food )
{
  favorite_food_ = favorite_food;
}
...
class TabbyCat : public Pet 
{
  ...
  void set_favorite_food( string favorite_food )
  {
    cout << "Tabbycats refuse to eat any food but meat! " << endl;
  }
};
